...
trait EmptyBoardFromFile extends Scope {
  ...
  var originalBoardPart = originalBoardString.split(Board.BOARD_GOALS_SEP)(0)
  var originalBoardPartLines = originalBoardPart.split(Board.END_OF_LINE)

  var originalGoalsPart = originalBoardString.split(Board.BOARD_GOALS_SEP)(1)
  var originalGoalsPartLines = originalGoalsPart.split(Board.END_OF_LINE)


  val board = Board.boardFromString(0, "empty", originalBoardString)
  var serialized = Board.boardToString(board)
...
class TestBoardSerialization extends Specification {
  "saving the empty (only border) board loaded from file" should {
    ...
    "keep the same number of lines for the specification of the board layout" in new EmptyBoardFromFile{
      ...
      var serializedBoardPart = serialized.split(Board.BOARD_GOALS_SEP)(0)
      var serializedLines = serializedBoardPart.split(Board.END_OF_LINE)
    ...
    "keep the same number of goals as the original" in new EmptyBoardFromFile{
      ...
      var serializedGoalsPart = serialized.split(Board.BOARD_GOALS_SEP)(1)
      var serializedGoalsLines = serializedGoalsPart.split(Board.END_OF_LINE)
