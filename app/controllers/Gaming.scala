...
object Gaming extends LocaleAwareSecureSocial {
  ...
  def currentGame(roomName: String) = UserAwareAction {
    implicit request =>
      DbRoom.findByName(roomName).map {
        room => {
          ...
          dbUser match {
            ...
            case Some(aUser) => {
              ...
              Ok(views.html.gaming.game(room, g, scores, aUser))
            }
          }
        }
  ...
  private def kick(username: String): Option[Unit] = {
    WsManager.roomForPlayer(username).map {
      r =>
  ...
  def eject(roomName: String, redirectTo: Option[String] = None) = UserAwareAction {
    implicit request =>
      kick(DbUser.fromRequest().get.fullName)
  ...
  //
  def getGame(roomName: String, gameId: String = null) = UserAwareAction {
    implicit request =>
      DbRoom.findByName(roomName).map {
        dbRoom =>
          DbGame.findByRoomAndId(roomName, gameId).map {
            game =>
              if (game.valid_until.getTime > System.currentTimeMillis()) {
                // game is active
                Redirect(routes.Gaming.currentGame(dbRoom.name))
              } else {
                Ok(views.html.gaming.gameFinished(dbRoom.name, gameId, DbUser.fromRequest()))
              }
          }.getOrElse(NotFound)
      }.getOrElse(NotFound)
  }
  ...
  //
  def gameStatus(roomName: String, gameId: String) = UserAwareAction {
    implicit request =>
      DbRoom.findByName(roomName).map {
        dbRoom =>
          Game.load(roomName, gameId).map {
            game =>
              var state = "playing"
              if (game.isDone) {
                state = "finished"
  ...
  private def playersAndScores(roomName: String, gameId: String): Seq[(String, Option[Int])] = {
    ...
    DbScore.findByGame(gameId).foreach {
      s =>
  ...
  //
  def gameScores(roomName: String, gameId: String) = UserAwareAction {
    implicit request =>
      DbRoom.findByName(roomName).map {
        dbRoom =>
          DbGame.findByRoomAndId(roomName, gameId).map {
  ...
  //
  def submitSolution(roomName: String, gameId: String) = UserAwareAction {
    implicit request =>
      DbRoom.findByName(roomName).map {
        dbRoom =>
          Game.load(roomName, gameId).map {
            game =>
              if (game.isDone) {
                ...
                val user = DbUser.fromRequest().get
                ...
                Form("solution" -> play.api.data.Forms.text).bindFromRequest.fold(
                  ...
                  solution => {
                    logMessage(roomName, user.fullName, "<SOLUTION>" + solution)
                    ...
                    if (game.validate(moves.map(parseMovement))) {
                      ...
                      try {
                        ...
                        notifyRoom(roomName, SOLUTION_FOUND, Seq[String](user.fullName, score.toString))
  ...
  def connectPlayer(roomName: String, player: String) = WebSocket.using[String] {
    implicit request =>
      ...
      val incomingPlayerChannel = Iteratee
        ...
        .mapDone {
        _ => // handle client closing connection
        //forget about that user in that room
          WsManager.room(roomName).map {
            r =>
