...
// Room controller
object Rooms extends LocaleAwareSecureSocial {

  // list the last n games of a room
  def gamesByRoom(roomName: String) = UserAwareAction {
    implicit request =>
  ...
  def previewGame(roomName: String, gameId: String) = UserAwareAction {
    implicit request =>
      DbRoom.findByName(roomName).map {
        dbRoom =>
          Game.load(dbRoom.name, gameId).map {
            g =>
