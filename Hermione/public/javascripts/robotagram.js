...
function moveRobot(direction) {
    var $robot = $(".selected"), originCell, destinationCell = null, previousDestination, nextDestination;
    if ($robot.length == 0) {
        // pas de robot
...
function initProgressBar(remainingTime, currentPercentage) {
    var remainingIntevrals = (remainingTime / 2000) + 1;
    var percentageIncrement = (100 - currentPercentage) / remainingIntevrals;
    doPollTimer(currentPercentage, percentageIncrement);
}

function doPollTimer(currentPercentage, percentageIncrement) {
    var newPercentage = currentPercentage + percentageIncrement;
    if (newPercentage >= 100) {
        setTimeout(timerEnd, 2000);
    } else {
        setTimeout(function() {
            doPollTimer(newPercentage, percentageIncrement)
        }, 2000);
