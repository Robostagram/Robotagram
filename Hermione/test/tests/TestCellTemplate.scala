...
class TestCellTemplate extends Specification {
  ...
  "render goal in cell template" in {
    var html = views.html.cell(EmptyCell.withGoal(new Goal(Color.Red,Symbol.Star)),null)
    ...
    html = views.html.cell(EmptyCell.withGoal(new Goal(Color.Blue,Symbol.Gear)),null)

    contentAsString(html) must contain("symbol")
    contentAsString(html) must contain("blue")
    contentAsString(html) must contain("gear")

    html = views.html.cell(EmptyCell.withGoal(new Goal(Color.Yellow,Symbol.Planet)),null)

    contentAsString(html) must contain("symbol")
    contentAsString(html) must contain("yellow")
    contentAsString(html) must contain("planet")

    html = views.html.cell(EmptyCell.withGoal(new Goal(Color.Green,Symbol.Moon)),null)

    contentAsString(html) must contain("symbol")
    contentAsString(html) must contain("green")
    contentAsString(html) must contain("moon")

    html = views.html.cell(EmptyCell.withGoal(new Goal(Color.Blue,Symbol.Sun)),null)
  ...
  "render robot in cells" in {
    var html = views.html.cell(EmptyCell,new Robot(Color.Red));
    contentType(html) must equalTo("text/html")
    contentAsString(html) must contain("robot")
    contentAsString(html) must contain("red")

    html = views.html.cell(EmptyCell,null);
